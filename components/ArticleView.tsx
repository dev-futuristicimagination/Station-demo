...
export const ArticleView: React.FC<ArticleViewProps> = ({ article, onBack }) => {
  useEffect(() => {
    ...
    // バックアップ
    const originalDescVal = metaDesc?.getAttribute('content') || '';
    const originalOgTitleVal = ogTitle?.getAttribute('content') || '';
    const originalOgDescVal = ogDesc?.getAttribute('content') || '';
    const originalOgImageVal = ogImage?.getAttribute('content') || '';

    let jsonLdScript: HTMLScriptElement | null = null;

    if (article) {
      // 1. タイトルの更新
      const pageTitle = `${article.title} | Asoventure Cheese`;
      ...
      if (ogImage && article.thumbnail) ogImage.setAttribute('content', article.thumbnail);

      // 4. 構造化データ (JSON-LD) の挿入 - Google検索結果でリッチリザルトを表示させるため
      const structuredData = {
        "@context": "https://schema.org",
        "@type": "Article",
        "headline": article.title,
        "image": [article.thumbnail || 'https://images.unsplash.com/photo-1557804506-669a67965ba0'],
        "datePublished": article.publishedAt.replace(/\./g, '-'), // YYYY.MM.DD -> YYYY-MM-DD
        "author": [{
          "@type": "Organization",
          "name": "Cheese Editorial Team",
          "url": "https://station-demo-alpha.vercel.app/"
        }],
    ...
    return () => {
      // クリーンアップ：ページを離れたら元のメタデータに戻す
      document.title = originalTitle;
      if (metaDesc) metaDesc.setAttribute('content', originalDescVal);
      if (ogTitle) ogTitle.setAttribute('content', originalOgTitleVal);
      if (ogDesc) ogDesc.setAttribute('content', originalOgDescVal);
      if (ogImage) ogImage.setAttribute('content', originalOgImageVal);
      if (ogUrl) ogUrl.setAttribute('content', window.location.origin); // URLはトップに戻す
